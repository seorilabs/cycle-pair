...
Cycle Pair는 두 사람(파트너)이 생리주기와 컨디션을 함께 이해하고 대비하도록 돕는 관계·컨디션 커뮤니케이션 도구다. 의료·진단·피임 도구가 아니며 가임기·배란·피임 표시 기능을 제공하지 않는다. 원본 건강 기록은 기본적으로 작성자 본인만 접근할 수 있고, 파트너에게는 사용자가 명시적으로 켠 항목만 별도 projection으로 제공된다.
