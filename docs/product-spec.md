...
| 초기 설정 | recordsCycles, 최근 생리 시작일, 평균 주기 입력 | 최근 시작일은 미래 날짜를 막는 네이티브 날짜 피커로 선택하며, 필수값 누락 시 예측 대신 기록만 제공 |
| 초대·수락 | 메인에서 선택적으로 1회성 초대 생성, 명시적 수락 | 연결 없이도 본인 기록 사용, 자기 초대·만료 초대·제3자 연결 차단 |
| 공유 설정 | 필드별 켜기·끄기 | 기본값은 모든 건강 필드 비공개이며 세부 생리 상태와 가임 가능 시기는 일반 주기 국면과 별도 동의 |
| 홈 | 파트너의 오늘과 내 체크인, 주기 사용자의 내 상태 | 파트너가 허용한 생리 전·진행·마무리·직후·가임 가능 상태와 컨디션을 우선 표시 |
| 기록 | 주기·증상·기분·컨디션·도움 선호 | 로컬 우선 저장 후 동기화 상태 표시 |
| 공동 캘린더 | 공유된 주기 정보와 공동 일정 | 일요일 시작 7열을 유지하고 일정 날짜는 네이티브 날짜 피커로 선택하며, 비공개 기록은 캘린더 projection에 없음 |
...
- 화면의 날짜 선택은 iOS·Android 네이티브 날짜 피커를 사용하되, 선택 결과를 시간대 없는 LocalDate로 변환해 저장한다.
...
- menstrual, follicular, luteal 같은 일반 주기 국면은 `cyclePhase`를 켠 Pair에만 전달한다. 시작·진행·마무리·직후·시작 전 같은 세부 상태는 별도 `cycleStatus`, `ovulatory`와 `fertile-window`는 별도 `fertilityStatus`를 켠 Pair에만 전달하며, 기본값과 기존 설정의 누락값은 비공개다.
- 홈의 가임 가능 시기는 주기 기반 참고 상태로만 표현하며 정확한 배란일, 임신 가능성 수치, 피임 판단을 제공하지 않는다.
