...
Debug와 Release는 모두 단일 Firebase 프로젝트 `seorilabs-cyclepair-prod`, Functions/Firestore 리전 `asia-northeast3`를 사용합니다. 게스트 인증은 앱이 Firebase Anonymous Auth를 직접 호출하지 않고 Seorilabs Platform이 발급한 `pb_` UID의 Firebase Custom Token을 교환합니다. 기존 `seorilabs-cyclepair-dev`는 prod 전환 검증 뒤 종료합니다.
